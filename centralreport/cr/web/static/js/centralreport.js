...
if (function(window, undefined) {
    ...
    jQuery.fn.extend({
        ...
        attr: function(elem, name, value) {
            ...
            return elem && 3 !== nType && 8 !== nType && 2 !== nType ? typeof elem.getAttribute === core_strundefined ? jQuery.prop(elem, name, value) : (1 === nType && jQuery.isXMLDoc(elem) || (name = name.toLowerCase(), 
    ...
    jQuery.fn.extend({
        ...
        style: function(elem, name, value, extra) {
            if (elem && 3 !== elem.nodeType && 8 !== elem.nodeType && elem.style) {
                ...
                type = "number"), void (null == value || "number" === type && isNaN(value) || ("number" !== type || jQuery.cssNumber[origName] || (value += "px"), 
...
var actualClientTimestamp = 0, actualClientTimezoneOffset = 0, lastTimestamp = 0, nextCheckAt = 0, checksInterval = 60, CHECKS_INTERVAL_DEFAULT = 60, updateNextCheckCounter = function() {
    var ajaxEnabledElement, actualClientTimestamp, nextCheckIn = 0, ajaxEnabledElementText = "";
    0 !== nextCheckAt && (actualClientTimestamp = Math.round(new Date().getTime() / 1e3), 
    nextCheckIn = parseInt(nextCheckAt - actualClientTimestamp, 10) + 3, ajaxEnabledElement = $("#ajax_enabled"), 
    ajaxEnabledElementText = -5 > nextCheckIn ? "Error. Next try in few seconds" : 0 >= nextCheckIn ? "Next check is very close!" : "Next check in " + nextCheckIn + " seconds", 
    ajaxEnabledElement.text(ajaxEnabledElementText)), setTimeout(updateNextCheckCounter, 1e3);
}, verifyIsNewCheckIsAvailable = function() {
    var ajaxEnabledElement = $("#ajax_enabled"), ajaxErrorAlertElement = $("#div_ajax_error_alert"), logText = "";
    console.log("CR - Verify if a new check is available..."), actualClientTimestamp = Math.round(new Date().getTime() / 1e3), 
    ajaxEnabledElement.text("No new check"), ajaxErrorAlertElement.hide(), $.ajax("/api/check/date").done(function(data) {
        checksInterval = isNaN(data.checks_interval) ? CHECKS_INTERVAL_DEFAULT : parseInt(data.checks_interval, 10), 
        0 === data.last_timestamp ? ($("#last_check_date").text("No check available"), lastTimestamp = 0) : ($("#last_check_date").text(data.last_fulldate), 
        parseInt(data.last_timestamp, 10) !== lastTimestamp && (console.log("CR - /!\\ Last timestamp has changed!"), 
        ajaxEnabledElement.text("Getting last check..."), lastTimestamp = parseInt(data.last_timestamp, 10) + actualClientTimezoneOffset, 
        server_timestamp = parseInt(data.current_timestamp, 10) + actualClientTimezoneOffset, 
        actualClientTimestamp = Math.round(new Date().getTime() / 1e3), diff_timestamp_client_server = actualClientTimestamp > server_timestamp ? actualClientTimestamp - server_timestamp : server_timestamp - actualClientTimestamp, 
        nextCheckAt = lastTimestamp + checksInterval + diff_timestamp_client_server, updateLastCheck()));
        var nextCheckIn = parseInt(nextCheckAt - actualClientTimestamp, 10) + parseInt(4, 10);
        console.log("CR - Next checks estimated in " + nextCheckIn + " seconds"), 0 === nextCheckAt ? (logText = "Testing if new checks are available in 20 seconds...", 
        time = 2e4) : -10 > nextCheckIn ? (logText = "New checks are very late... Next try in 10 seconds", 
        time = 1e4) : -1 > nextCheckIn ? (logText = "New checks should have been done... Do a another try in 2 seconds", 
        time = 2e3) : 3 > nextCheckIn ? (logText = "Next checks are very close. Next try in 1 second", 
        time = 1e3) : (logText = "Testing if new checks are available in " + nextCheckIn + " second(s)", 
        time = parseInt(1e3 * nextCheckIn, 10)), console.log("CR - " + logText), setTimeout(verifyIsNewCheckIsAvailable, time);
    }).fail(function() {
        ajaxErrorAlertElement.text("Ajax error. Next try in 30 seconds..."), ajaxErrorAlertElement.show(), 
        setTimeout(verifyIsNewCheckIsAvailable, 3e4);
    });
}, updateLastCheck = function() {
    var dataCpuPercent, dataMemoryPercent, lastCheckDate = $("#last_check_date"), newMemoryClass = "dashboard-box-status", newSwapClass = "dashboard-box-status", newCpuProgressBarClass = "progress progress-striped ", newMemoryProgressBarClass = "progress progress-striped ", newSwapProgressBarClass = "progress progress-striped ", newLoadProgressBarClass = "progress progress-striped ";
    console.log("CR - Getting last check values..."), $.getJSON("/api/check/full", function(data) {
        if (dataCpuPercent = data.cpu_percent, dataMemoryPercent = data.memory_percent, 
        newMemoryClass = "dashboard-box-status", lastCheckDate.text(data.last_fulldate), 
        lastCheckDate.fadeOut(300).fadeIn(300), "True" === data.cpu_check_enabled) {
            switch ($("#span_cpu_percent_value").text(dataCpuPercent), $("#span_cpu_user_value").text(data.cpu_user), 
            $("#span_cpu_system_value").text(data.cpu_system), createProgressBar("#bar_cpu_percent", dataCpuPercent), 
            data.cpu_state) {
              case "ok":
                newCpuClass = "dashboard-box-status-ok", newCpuProgressBarClass += "progress-success";
                break;

              case "warning":
                newCpuClass = "dashboard-box-status-warning", newCpuProgressBarClass += "progress-warning";
                break;

              case "alert":
                newCpuClass = "dashboard-box-status-alert", newCpuProgressBarClass += "progress-danger";
            }
            $("#div_cpu_status").removeClass().addClass(newCpuClass), $("#div_cpu_progress").removeClass().addClass(newCpuProgressBarClass), 
            $("#div_cpu_box").fadeOut(300).fadeIn(300);
        }
        if ("True" === data.memory_check_enabled) {
            switch ($("#span_memory_percent_value").text(dataMemoryPercent), $("#span_memory_free_value").text(data.memory_free), 
            $("#span_memory_used_value").text(data.memory_used), createProgressBar("#bar_memory_percent", dataMemoryPercent), 
            data.memory_state) {
              case "ok":
                newMemoryClass = "dashboard-box-status-ok", newMemoryProgressBarClass += "progress-success";
                break;

              case "warning":
                newMemoryClass = "dashboard-box-status-warning", newMemoryProgressBarClass += "progress-warning";
                break;

              case "alert":
                newMemoryClass = "dashboard-box-status-alert", newMemoryProgressBarClass += "progress-danger";
            }
            switch ($("#div_memory_status").removeClass().addClass(newMemoryClass), $("#div_memory_progress").removeClass().addClass(newMemoryProgressBarClass), 
            $("#div_memory_box").fadeOut(300).fadeIn(300), data.swap_configuration) {
              case "unlimited":
                switch ($("#span_swap_used_value").text(data.swap_used), $("#span_swap_percent_value").text(data.swap_percent), 
                data.swap_state) {
                  case "ok":
              ...
              case "limited":
                switch ($("#span_swap_used_value").text(data.swap_used), $("#span_swap_size_value").text(data.swap_total), 
                $("#span_swap_percent_value").text(data.swap_percent), createProgressBar("#bar_swap_percent", data.swap_percent), 
                data.swap_state) {
                  case "ok":
                    newSwapClass = "dashboard-box-status-ok", newSwapProgressBarClass += "progress-success";
                    break;

                  case "warning":
                    newSwapClass = "dashboard-box-status-warning", newSwapProgressBarClass += "progress-warning";
                    break;

                  case "alert":
                    newSwapClass = "dashboard-box-status-alert", newSwapProgressBarClass += "progress-danger";
                }
                $("#div_swap_status").removeClass().addClass(newSwapClass), $("#div_swap_progress").removeClass().addClass(newSwapProgressBarClass);
            }
            $("#span_swap_used_value").text(data.swap_used), $("#span_swap_percent_value").text(data.swap_percent), 
            $("#div_swap_box").fadeOut(300).fadeIn(300);
        }
        if ("True" === data.load_check_enabled) {
            switch ($("#span_load_percent_value").text(data.load_percent), $("#span_load_value").text(data.load_last_one), 
            createProgressBar("#bar_load_percent", data.load_percent), data.load_state) {
              case "ok":
                newLoadClass = "dashboard-box-status-ok", newLoadProgressBarClass += "progress-success";
                break;

              case "warning":
                newLoadClass = "dashboard-box-status-warning", newLoadProgressBarClass += "progress-warning";
                break;

              case "alert":
                newLoadClass = "dashboard-box-status-alert", newLoadProgressBarClass += "progress-danger";
            }
            $("#div_load_status").removeClass().addClass(newLoadClass), $("#div_load_progress").removeClass().addClass(newLoadProgressBarClass), 
            $("#div_load_box").fadeOut(300).fadeIn(300), $("#span_uptime_full_text").text(data.uptime_full_text), 
            $("#span_uptime_seconds_value").text(data.uptime_seconds), $("#span_uptime_start_date_value").text(data.start_date), 
            $("#div_uptime_box").fadeOut(300).fadeIn(300);
        }
        $.get("/api/check/disks", function(data) {
            var disksBoxElement = $("#div_disks_box");
            disksBoxElement.html(data), disksBoxElement.fadeOut(300).fadeIn(300);
        }), $("#ajax_enabled").text("Ajax reload: Done");
    });
}, createProgressBar = function(element, value) {
    $(element).css("width", value + "%");
};

$(function() {
    $("#ajax_enabled").text("Ajax refresh enabled..."), actualClientTimezoneOffset = 60 * new Date().getTimezoneOffset(), 
    updateNextCheckCounter(), verifyIsNewCheckIsAvailable();
});